...
const TrandingPage = () => {
  ...
  const displayedRecipes = trandingRecipes.slice(
   ...
   return (
     <div className="relative bg-gradient-to-tl from-blue-500 via-wavy-purple to-accent-400 min-h-screen">
         ...
         {loading ? (
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
             {displayedRecipes.map((recipe, index) => (
               <motion.div
                 key={recipe.id}
                 className="border-2 p-4 bg-wavy-to-br from-purple-700 to-accent-600 rounded-lg
               shadow-md hover:shadow-lg
                ...
                text-center mb-4"
                 ...
                 exit={{ opacity: 1 }}
                 whileHover={{ scale: 1, opacity: 1, rotate: 360 }}
               >
                 ...
                 <p className="text-accent-900 mb-2">
                   {recipe.description.length > 30
                     ? `${recipe.description.slice(0, 30)}...`
                     : recipe.description}
                 </p>

                 <p className="text-yellow-500 font-bold mb-2">Author: {recipe.author}</p>
                 <Link
                   to={`/recipes/${recipe.id}`}
                   className="text-white px-2 hover:bg-yellow-500 border rounded"
                 >
                   Lihat Resep
                 </Link>
               </motion.div>
